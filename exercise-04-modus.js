...
function cariModus(arr) {
  var modus = 0, max = 0, mod = 0, count = 0;

  for (var i = 0; i < arr.length; i++) {
    count = 0;
    // menghitung jumlah kemunculan dari tiap2 elemen
    for (var j = 0; j < arr.length; j++) {
      if (arr[i] == arr[j]) {
        count++;
      }
    }
    // mencari jumlah kemunculan terbanyak
    if (count > max) {
      max = count;
      // mengambil elemen yg muncul terbanyak
      mod = arr[i];
    }
  }

  // jika jumlah kemunculan sama dengan panjang arr maka modus -1
  // jika jumlah kemunculan terbesar adalah 1, maka modus -1
